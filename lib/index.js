...
const { Address, toBytes } = require('@ethereumjs/util')
...
const processTransaction = async (tx) => {
	...
	// get sender
	const txKey = Buffer.from(tx.key.substring(2), 'hex')
	const senderWallet = Wallet.fromPrivateKey(txKey)
	...
	// debug
	console.log(`tx.action=${tx.action}`, `rollupId=${rollupId}`, `senderWallet=${senderWallet.getAddressString()}`, `senderNonce=${senderNonce}`)
	...
	if (tx.action === 'add_rollup') {
		...
		const [code] = tx.params

		// get address
		const address = Address.generate(senderAddress, BigInt(senderNonce))
		console.log('|-', 'result', `address=${address.toString()}`)
