...
// state
const executionState = { rollup: { vm: new VM() }, hub: { count: 0, contracts: {} } }
...
const reassignContract = async (createdAddress, rollupId) => {
	...
	const rollupTo = executionState['rollup']
	const rollupFrom = executionState['rollup']
...
const processTransaction = async (tx) => {
	if (tx.type === 'hub') {
		if (tx.action === 'add_rollup') {
			...
			// add rollup
			const rollupId = executionState['hub'].count // get new
			executionState['hub'].count += 1
			...
			const rollupCount = executionState['hub'].count
			...
			// remove rollup
			executionState['rollup'] = undefined
			
		} else if (tx.action === 'create_contract') {

			// get rollup
			const rollupId = executionState['hub'].count - 1 // get last
			const rollup = executionState['rollup']
...
const queryState = async (address) => {
	...
	const storage = await executionState.rollup.vm.stateManager.dumpStorage(Address.fromString(address))
	return storage
}

const queryHub = async (rollupId) => {
	return executionState['rollup']
}
