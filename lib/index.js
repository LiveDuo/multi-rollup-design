...
const queryState = async (address) => {
	const rollupId = executionLayer['hub'].contracts[address.toString()].rollupId
	const storage = await executionLayer.rollups[rollupId].vm.stateManager.dumpStorage(address)
	return storage
}
