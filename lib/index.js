...
const processTransaction = async (tx) => {
	...
	if (tx.action === 'add_rollup') {
		...
		const address = Address.generate(senderAddress, BigInt(nonceCount))
		...
		if (rollupId === lastRollupId) {
			...
			result = await rollupState.vm.runTx({ tx: signedTx2, skipBalance: true, skipNonce: true })
		}

		// update hub
		const contractAddress = result.createdAddress ?? address
		hubState.contracts[contractAddress] = { rollupId: lastRollupId }

		return { createdAddress: contractAddress }
