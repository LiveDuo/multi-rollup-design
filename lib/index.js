...
const reassignContract = async (address, rollupId) => {
	...
	const account = await rollupFrom.vm.stateManager.getAccount(address).then((a) => new Account(BigInt(a.nonce), BigInt(a.balance)))
	await rollupTo.vm.stateManager.putAccount(address, account)

	// assign contract code to new rollup
	const code = await rollupFrom.vm.stateManager.getContractCode(address)
	await rollupTo.vm.stateManager.putContractCode(address, code)

	// assign contract storage to new rollup
	const storage = await rollupFrom.vm.stateManager.dumpStorage(address)
	for (const [k, v] of Object.entries(storage)) {
		const key = new Uint8Array(Buffer.from(k.substring(2), 'hex'))
		const value = new Uint8Array(Buffer.from(v.substring(2), 'hex'))
		await rollupTo.vm.stateManager.putContractStorage(address, key, value)
	}

	// TODO if current rollup id
	// remove state from rollup
	await rollupFrom.vm.stateManager.clearContractStorage(address)
}
...
const processTransaction = async (tx) => {
	...
	if (tx.action === 'add_rollup') {
		...
		// reassign contract
		const [_, address, targetRollupId] = tx.params
		await reassignContract(Address.fromString(address), targetRollupId)
