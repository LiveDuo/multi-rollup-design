...
// state
const executionState = { rollups: {}, hub: { contracts: {} } }
...
const processTransaction = async (tx) => {
	if (tx.type === 'hub') {
		if (tx.action === 'add_rollup') {
			...
			// add rollup
			const rollupId = Object.keys(executionState.rollups).length // get new
			executionState['rollups'][rollupId] = { vm: new VM() }
			...
			const rollupCount = Object.keys(executionState.rollups).length
			const contracts = Object.entries(executionState['hub'].contracts).filter(([_, contract]) => contract.rollupId === rollupId)
			for (const [i, address] of contracts.map(([address, _]) => address).entries()) {
				const targetRollupId = i % rollupCount
				await reassignContract(executionState, Address.fromString(address), targetRollupId)
			}

			// remove rollup
			executionState['rollups'][rollupId] = undefined
			
		} else if (tx.action === 'create_contract') {

			// get rollup
			const rollupId = Object.keys(executionState.rollups).length - 1 // get last
			const rollup = executionState['rollups'][rollupId]
			...
			// update hub
			executionState['hub'].contracts[result.createdAddress] = { rollupId } // code: tx.data
			...
			await reassignContract(executionState, Address.fromString(createdAddress), rollupId)
		...
		if (tx.action === 'call_contract') {
			...
			const rollupId = executionState['hub'].contracts[contractAddress].rollupId
			const rollup = executionState['rollups'][rollupId]
...
const queryState = async (address) => {
	const rollupId = executionState['hub'].contracts[address].rollupId
	const storage = await executionState.rollups[rollupId].vm.stateManager.dumpStorage(Address.fromString(address))
	return storage
}

const queryHub = async (rollupId) => {
	return executionState['rollups'][rollupId]
}

const debug = async () => {
	for ([address, data] of Object.entries(executionState.hub.contracts)) {
		const storage = await executionState.rollups[data.rollupId].vm.stateManager.dumpStorage(Address.fromString(address))
