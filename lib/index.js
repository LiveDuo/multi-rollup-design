...
const { Address, toBytes, ecrecover, bytesToBigInt } = require('@ethereumjs/util')
...
const processTransaction = async (tx) => {
	...
	const v = signature.subarray(0, 1)
	...
	// debug
	console.log(`tx.action=${tx.action}`, `rollupId=${rollupId}`, `senderAddress=${senderAddress.toString()}`, `senderNonce=${senderNonce}`)
