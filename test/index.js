...
test('create 2 contracts and reassign one of them', async () => {
    ...
    const result3 = await queryState(0, result.createdAddress)
    assert.strictEqual(Object.values(result3)[0], '0x02')

    // call contract 1
	await submitTransaction({ type: 'rollup', typeParams: [0], action: 'call_contract', actionParams: [result.createdAddress], data: '' })
    const result4 = await queryState(0, result.createdAddress)
    ...
    const result5 = await queryState(1, result2.createdAddress)
    ...
    // call contract 2
    const result6 = await queryState(0, result2.createdAddress)
