...
test('integration: create 2 contracts and reassign one of them', async () => {
	...
	const createResult = await submitTransaction({ type: 'hub', action: 'create_contract', params: [rollupId, '0x' + code.join('')] })
    
    // create contract 2
	const code2 = [OP_CODES.PUSH1, '04', OP_CODES.PUSH1, '05', OP_CODES.SSTORE]
	const createResult2 = await submitTransaction({ type: 'hub', action: 'create_contract', params: [rollupId, '0x' + code2.join('')] })

	// call contract 1
	await submitTransaction({ type: 'rollup', action: 'call_contract', params: [rollupId, createResult.createdAddress.toString(), []] })
    ...
    // call contract 2
	await submitTransaction({ type: 'rollup', action: 'call_contract', params: [rollupId, createResult2.createdAddress.toString(), []] })
