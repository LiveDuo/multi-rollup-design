...
test('integration: create 2 contracts and reassign one of them', async () => {
	...
	const createResult = await submitTransaction({ type: 'hub', action: 'create_contract', params: ['0x' + code.join('')] })
    
    // create contract 2
	const code2 = [OP_CODES.PUSH1, '04', OP_CODES.PUSH1, '05', OP_CODES.SSTORE]
	const createResult2 = await submitTransaction({ type: 'hub', action: 'create_contract', params: ['0x' + code2.join('')] })
    ...
    // // remove rollup
	// await submitTransaction({ type: 'hub', action: 'remove_rollup', data: [1] })
